...
namespace Quicksilver {
    public class UITextField : UIElement {
        ...
        public UIAlignment TextAlignment {get; set;}
        public bool Editable {get; set;}
        public bool AllowLineBreaks {get; set;}
        public bool AllowNonNumeric {get; set;}
        public Action Action {get; set;}
        public bool Selected {get; private set;}
        public string Text {get; set;}

        public UITextField(FloatRect bounds, UIAlignment alignment, string ghostText, uint charSize, UIAlignment textAlignment, bool allowLineBreaks, bool allowNonNumeric, Action action) {
            Bounds = bounds;
            Alignment = alignment;
            GhostText = ghostText;
        ...
        public override void Run(RenderWindow window, Vector2f mousePos) {
            ...
            if (Text == "" && !Selected) {
                using (Text text = new Text()) {
                    ...
                    if ((int)TextAlignment % 3 == 1) {
                        extraPos.X = TrueBounds.Size.X / 2;
                        extraOrig.X = text.GetGlobalBounds().Size.X / 2;
                    }
                    if ((int)TextAlignment % 3 == 2) {
                        extraPos.X = TrueBounds.Size.X - 10;
                        extraOrig.X = text.GetGlobalBounds().Size.X;
                    }
                    if ((int)TextAlignment / 3 == 1) {
                        extraPos.Y = TrueBounds.Size.Y / 2;
                        extraOrig.Y = text.GetGlobalBounds().Size.Y / 2;
                    }
                    if ((int)TextAlignment / 3 == 2) {
                        extraPos.Y = TrueBounds.Size.Y - 10;
                        extraOrig.Y = text.GetGlobalBounds().Size.Y;
                    }
                ...
                using (Text text = new Text()) {
                    text.Font = Program.Font;
                    text.DisplayedString = Text + (Selected ? "|" : "");
                    text.CharacterSize = CharacterSize;
                    Vector2f extraPos = new Vector2f(10, 10);
                    Vector2f extraOrig = new Vector2f(0, 0);
                    if ((int)TextAlignment % 3 == 1) {
        ...
        public override void OnKeyPressed(KeyEventArgs e) {
            if (Selected && Editable) {
                if (e.Code >= Keyboard.Key.Num0 && e.Code <= Keyboard.Key.Num9) {
                    Text += e.Shift && AllowNonNumeric ? ")!@#$%^&*("[(int)e.Code-26] : (char)('0'+(int)e.Code-26);
                }
                if (AllowNonNumeric) {
                ...
                if (e.Code == Keyboard.Key.Enter) {
                    if (e.Shift && AllowLineBreaks && AllowNonNumeric) {
                        Text += "\n";
                    }
                    if (!e.Shift) {
                        Action();
